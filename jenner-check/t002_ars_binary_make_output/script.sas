/****************************************************************************
* jenner-check bundle: t007_ars_binary_make_output
*
* Exercises this repo's binary-endpoint report builder, unmodified:
*   - %ars_binary_make_output
*
* This macro reshapes an ARS-normalized Fisher's-exact long dataset (the kind
* produced by %ars_binary_production_analysis) into report-ready
* col0/col1/col2 rows: event / non-event counts with percentages, the odds
* ratio, its 95% CI, and the Fisher p-value. The macro definition below is
* copied verbatim from ars_binary_make_output.sas. A small mock ARS long
* dataset built from the operation IDs %ars_binary_production_analysis emits
* (OP_BIN_COUNT_PCT, OP_BIN_OR) stands in for its output.
****************************************************************************/

/* ===== ars_binary_make_output.sas (verbatim) ======================== */
/****************************************************************************
...
/* ===== mock ARS long binary results + driver ======================== */
/* Newly initiated renal replacement therapy by Day 28, two arms.        */
data work.ars_binary_long;
    length source_system $8 analysis_id $40 operation_id $40 operation_role $20
           parameter_cd $40 parameter $200 treatment $200 contrast $400
           grouping_1_id $40 grouping_1_value $200 display_id $80;
    length result_numeric 8;
    infile datalines dsd truncover;
    input operation_id $ operation_role $ treatment $ grouping_1_value $ result_numeric;

    source_system='SAS';
    analysis_id='AN_BINARY_FISHER';
    parameter_cd='ORGA28';
    parameter='Newly Initiated RRT by Day 28';
    display_id='T_RRT';
    grouping_1_id='RESP_CAT';
    datalines;
OP_BIN_COUNT_PCT,COUNT,AON-D21,EVENT,6
OP_BIN_COUNT_PCT,PCT,AON-D21,EVENT,10.0
OP_BIN_COUNT_PCT,COUNT,AON-D21,NON_EVENT,54
OP_BIN_COUNT_PCT,PCT,AON-D21,NON_EVENT,90.0
OP_BIN_COUNT_PCT,COUNT,Placebo,EVENT,15
OP_BIN_COUNT_PCT,PCT,Placebo,EVENT,25.9
OP_BIN_COUNT_PCT,COUNT,Placebo,NON_EVENT,43
OP_BIN_COUNT_PCT,PCT,Placebo,NON_EVENT,74.1
OP_BIN_OR,ESTIMATE,,AON-D21 vs Placebo,0.318
OP_BIN_OR,LCL,,AON-D21 vs Placebo,0.114
OP_BIN_OR,UCL,,AON-D21 vs Placebo,0.889
OP_BIN_OR,PVALUE,,AON-D21 vs Placebo,0.0284
;
run;

%ars_binary_make_output(
    arsds=work.ars_binary_long,
    out=work.page_binary,
    test_value=AON-D21,
    ref_value=Placebo,
    test_hdr=AON-D21,
    ref_hdr=Placebo,
...
title "ARS binary endpoint report rows (col0/col1/col2)";
proc print data=work.page_binary noobs;
